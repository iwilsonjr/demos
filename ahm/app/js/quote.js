...
// End Datepicker
  ...
  $(".help, .ajaxHelp").on("click focus", function(){ 
	...
	//Initialization 
	adultCounter($("#numberTravellers").val());

	//Select number of adults
	$("#numberTravellers").change(function(){
	...
	//Initialization 
	$("#hasPromoCode").change(function(){
		if($(this).is(':checked')){
			$("#enterCode").removeClass("hide");
		} else {
			$("#enterCode").addClass("hide");			
		}
	});
...
function adultCounter(test){
	if (test > 0) {
		...
		$(".ageHolder li:lt(" + test + ")").removeClass("hide");
	} else {
