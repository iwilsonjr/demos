...
(function($) {
	...
	var defaults = {
		...
		//uberSelection settings
		uberSelection: false,
		uberID: null,
		uberText: 'All Selections'
	},
	openXHR = {},
	buildItems = function($this, data, settings) {
		...
		if (settings.builder) {
			...
			if (data) {
				...
				if (settings.uberSelection) { //Select to view all choices
					str.push('<li ' + vclass + ' data-icon=' + settings.icon + '><a href="' + settings.link + settings.uberID + '" data-transition="' + settings.transition + '">' + settings.uberText + '</a></li>'); 
				}			
			}
		}
		if ($.isArray(str)) {
