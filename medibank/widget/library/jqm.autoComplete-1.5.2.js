...
(function($) {
	...
	buildItems = function($this, data, settings) {
		...
		if (settings.builder) {
			...
			if (data) {
				...
				str.push('<li ' + vclass + ' data-icon=' + settings.icon + '><a href="#all" data-transition="' + settings.transition + '">---  See All Regions and Countries  ---</a></li>'); //Addition for AU Post			
			}
		}
		if ($.isArray(str)) {
			str = str.join('');
	...
	attachCallback = function(settings) { // happend when user clicks on something in the auto list
		$('li a', $(settings.target)).bind('click.autocomplete',function(e){
			// added this for my validation kh			
			e.stopPropagation();
			e.preventDefault();
			settings.callback(e);
			//alert("test");
			...
			//alert($("input[name=regionID]:checked").val());
			//alert($("input[name=destinationCountryID]:checked").val());
