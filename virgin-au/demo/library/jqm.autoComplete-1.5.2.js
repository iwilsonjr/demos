	...
	buildItems = function($this, data, settings) {
		...
		if (settings.builder) {
			...
			if (data) {
				...
				str.push('<li ' + vclass + ' data-icon=' + settings.icon + '><a href="#all" data-transition="' + settings.transition + '">---  See All Regions and Countries  ---</a></li>'); //Addition for Virgin Australia			
			}
		}
		if ($.isArray(str)) {
			str = str.join('');
